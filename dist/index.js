...
server.tool("scout_navigate", "Navigate to a URL. Returns a snapshot with accessibility tree (numbered elements) and screenshot with visual badges.", { url: z.string().url().describe("URL to navigate to") }, async ({ url }) => {
    const result = await navigateTool(url);
    return {
        content: [
            {
                type: "text",
                text: JSON.stringify({ url: result.url, title: result.title, timestamp: result.timestamp, elements: result.elements, markdown: result.markdown }, null, 2),
            },
            ...(result.screenshot
...
server.tool("scout_snapshot", "Take a snapshot of the current page. Returns accessibility tree (numbered elements) and screenshot with visual badges.", {}, async () => {
...
server.tool("scout_elements", "Get the current page's accessibility tree (numbered elements) without taking a screenshot. Faster than scout_snapshot.", {}, async () => {
...
server.tool("scout_screenshot", "Take a plain screenshot of the current page without element badges.", {}, async () => {
...
// Scroll the page
server.tool("scout_scroll", "Scroll the page in a direction.", {
    direction: z
...
server.tool("scout_select", "Select an option from a dropdown/select element by its snapshot ID.", {
    id: z.number().int().positive().describe("Element ID from the last snapshot"),
...
// Wait for condition
server.tool("scout_wait", "Wait for a page condition.", {
    condition: z
...
// Hover element
server.tool("scout_hover", "Hover over an element by its snapshot ID.", {
    id: z.number().int().positive().describe("Element ID from the last snapshot"),
}, async ({ id }) => {
...
// Press key
server.tool("scout_press_key", "Press a keyboard key (e.g. Enter, Escape, ArrowDown, Backspace).", {
...
// Navigation
server.tool("scout_back", "Go back in browser history.", {}, async () => {
    const result = await backTool();
    return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
});
server.tool("scout_forward", "Go forward in browser history.", {}, async () => {
    const result = await forwardTool();
    return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
});
server.tool("scout_refresh", "Refresh the current page.", {}, async () => {
    const result = await reloadTool();
    return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
});
// Resource blocking
server.tool("scout_block_resources", "Block certain resource types to speed up loading (e.g. image, media, stylesheet, font, script).", {
...
// Drag and drop
server.tool("scout_drag", "Drag one element onto another.", {
...
// Session management
server.tool("scout_save_session", "Save the current browser session (cookies, localStorage) to a named file.", {
...
server.tool("scout_load_session", "Load a saved browser session. This will restart the browser context.", {
...
server.tool("scout_handoff", "Inject a banner in the live browser asking the user to take a manual action. Returns IMMEDIATELY with a handoff_id — does NOT block. Poll scout_handoff_check(handoff_id) every 5-10 seconds until status is 'completed'. Use for: CAPTCHAs, SMS codes, authenticator app prompts, email verification.", {
...
server.tool("scout_handoff_check", "Check whether a pending handoff has been completed by the human. Returns immediately. Call this every 5-10 seconds after scout_handoff until status is 'completed' or 'expired'.", {
...
server.tool("scout_handoff_cancel", "Cancel a pending handoff and remove the banner from the browser.", {
    handoff_id: z.string().describe("The handoff_id returned by scout_handoff"),
}, async ({ handoff_id }) => {
...
// Tab management
server.tool("scout_tabs", "List all open browser tabs with their index, URL, title, and which is active.", {}, async () => {
    const tabs = await tabsTool();
    return { content: [{ type: "text", text: JSON.stringify(tabs, null, 2) }] };
});
server.tool("scout_switch_tab", "Switch to a browser tab by index (from scout_tabs).", {
...
server.tool("scout_new_tab", "Open a new browser tab, optionally navigating to a URL.", {
