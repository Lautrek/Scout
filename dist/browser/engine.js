...
class BrowserEngine {
    ...
    async setBlockedResources(types) {
    ...
    async switchPage(indexOrUrl) {
        ...
        if (!target) {
            throw new Error(`No tab found for: ${indexOrUrl}. Open tabs: ${pages.map((p, i) => `[${i}] ${p.url()}`).join(", ")}`);
        }
    ...
    async restartContext(storageStatePath) {
        if (this.context) {
            await this.context.close();
            this.context = null;
        }
    ...
    async _ensureBrowser(storageStatePath) {
        // Persistent context: already running
        if (this._persistent && this.context && !storageStatePath)
            return;
        // Non-persistent: already connected
        if (!this._persistent && this.browser?.isConnected() && this.context && !storageStatePath)
            return;
            ...
            if (existingPort) {
                try {
                    if (!this.browser?.isConnected()) {
                        this.browser = await chromiumExtra.connectOverCDP(`http://localhost:${existingPort}`, { timeout: 3000 });
                    }
