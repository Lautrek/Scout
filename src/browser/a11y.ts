...
let _lastElements: ScoutElement[] = [];

export function getElement(id: number): ScoutElement | undefined {
  return _lastElements.find((e) => e.id === id);
}

export function setLastElements(elements: ScoutElement[]): void {
...
export async function extractA11yElements(page: Page): Promise<ScoutElement[]> {
  const MAX = MAX_ELEMENTS;

  const domElements = await page.evaluate((max: number) => {
    // Clear previous IDs
    document.querySelectorAll("[data-scout-id]").forEach((el) =>
      el.removeAttribute("data-scout-id")
    );
    ...
    document.querySelectorAll(SELECTORS).forEach((el) => {
      ...
      if (ariaLabelledBy) {
        for (const id of ariaLabelledBy.split(/\s+/)) {
          ...
          if (ref) labelledText += ref.textContent?.trim() + " ";
        }
      ...
      if (!label && !nonLabelledInputRoles.has(role)) return;
  ...
  const elements: ScoutElement[] = domElements.map((d) => ({
    id: d.id,
    role: d.role,
    label: d.label,
