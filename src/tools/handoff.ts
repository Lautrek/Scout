...
const _store = new Map<string, HandoffState>();
...
export async function handoffTool(
  instruction: string,
  timeoutMs = 300_000
): Promise<{ handoff_id: string; status: HandoffStatus; message: string }> {
  const id = randomUUID();
  ...
  async function injectBanner(): Promise<void> {
    await page.evaluate(
      ({ msg, handoffId }: { msg: string; handoffId: string }) => {
  ...
  state.navHandler = navHandler as any;
  ...
  (async () => {
    ...
    while (Date.now() < deadline) {
      ...
      try {
        const done = await page.evaluate(
          (hid: string) =>
            (window as any).__scout_handoff_done__ === hid ||
            localStorage.getItem("__scout_handoff_done__") === hid,
          id
        );
        if (done) {
          s.status = "completed";
    ...
    await page.evaluate(() => {
      ...
      delete (window as any).__scout_handoff_done__;
    }).catch(() => {});
  })();
 ...
 */
export function checkHandoff(handoff_id: string): {
  status: HandoffStatus;
  elapsed_s: number;
  instruction?: string;
  message: string;
 ...
 */
export async function cancelHandoff(handoff_id: string): Promise<{ cancelled: boolean }> {
  const s = _store.get(handoff_id);
